...
return function(stmt_or_value <opt_i>)
  ...
  elseif t == sql.SQLITE_BLOB then
    local b = {}
    b.len = sql.sqlite3_<variant>_bytes(stmt_or_value <opt_i>)
    b.data = ffi.new(ffi.typeof("char[?]"), b.len)
    setmetatable(b, {__len = function(t) return t.len end})
    ffi.copy(b.data, sql.sqlite3_<variant>_blob(stmt_or_value <opt_i>), b.len)
    return b
...
return function(stmt_or_value, v <opt_i>)
  ...
  elseif t == "table" and getmetatable(v) == blob_mt then
    v = v[1]
    return sql.sqlite3_<variant>_blob(stmt_or_value <opt_i>, v.data, v.len,
      transient)
  elseif t == "nil" then
